...
{
	// Creamos el TFDQuery que gestionará el insert
	std::unique_ptr<TFDQuery> LQry(new TFDQuery(nullptr));
	LQry->Connection = this->connEmployees;
...
bool TDmMain::UpdateCountry(const System::UnicodeString AName, const System::UnicodeString ACurrency)
{
	// Creamos el TFDQuery que gestionará el update
	std::unique_ptr<TFDQuery> LQry(new TFDQuery(nullptr));
	LQry->Connection = this->connEmployees;
	LQry->SQL->Text = "update country set currency = :currency where country = :country";
	LQry->Params->ParamByName("country")->AsString = AName;
	LQry->Params->ParamByName("currency")->AsString = ACurrency;
	try {
		LQry->ExecSQL();
...
bool TDmMain::DeleteCountry(const System::UnicodeString AName)
{
	// Creamos el TFDQuery que gestionará la consulta
	std::unique_ptr<TFDQuery> LQry(new TFDQuery(nullptr));
	LQry->Connection = this->connEmployees;
	LQry->SQL->Text = "delete from country where country = :country";
	LQry->Params->ParamByName("country")->AsString = AName;
	try {
		LQry->ExecSQL();
		return true;
